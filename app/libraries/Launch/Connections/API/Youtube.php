...
class YoutubeAPI extends GoogleAPI
{
    ...
    public function postContent($content) {
        ...
        try {
            ...
            $response['external_id'] = $status->id;
        } catch (\Exception $e) {
    ...
    public function updateStats($accountConnectionId)
    {
        ...
        $response = $youtube->videos->listVideos(
            'statistics',
            ['id' => implode(',', array_keys($content))]
        );

        $count = 0;
        foreach ($response as $video) {
            if (isset($content[$video->id])) {
                $count++;
                $c = $content[$video->id];

                $stats = $video['statistics'];

                $c->pivot->likes = $stats['likeCount'] - $stats['dislikeCount'] + $stats['favoriteCount'] * 10;
                $c->pivot->comments = $stats['commentCount'];
                $c->pivot->views = $stats['viewCount'];
                $c->pivot->save();
            }
        }
