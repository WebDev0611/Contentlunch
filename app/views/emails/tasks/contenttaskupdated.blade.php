...
<title>A Task has been updated on November 28th, 2014 at 3:47pm</title>
								...
								<tr style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; box-sizing: border-box; font-size: 14px; margin: 0; padding: 0;">
									<td class="content-block" style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; box-sizing: border-box; font-size: 14px; vertical-align: top; margin: 0; padding: 10px 0 10px;" valign="top">
										@if ($orignalTaskName == $taskName && $orignalTaskDueDate != $taskDueDate)
											The due date for <i><strong style='color: #6da5ff;'>{{ $taskName }}</strong></i> on the <a style="text-decoration: none;" href="{{ $contentUrl }}"><strong style="color: #990000">{{ $contentTitle }}</strong></a> content has been change from {{ $orignalTaskDueDate }} to <strong>{{ $taskDueDate }}</strong>.
										@elseif ($orignalTaskName != $taskName && $orignalTaskDueDate == $taskDueDate)
											The task <i>{{ $orignalTaskName }}</i> on the <a style="text-decoration: none;" href="{{ $contentUrl }}"><strong style="color: #990000">{{ $contentTitle }}</strong></a> content has been changed to <i><strong style='color: #6da5ff;'>{{{ $taskName }}}</strong></i>. The due date remains the same at <strong>{{ $taskDueDate }}</strong>.
										@elseif ($orignalTaskName != $taskName && $orignalTaskDueDate != $taskDueDate)
											The task <i>{{ $orignalTaskName }}</i> on the <a style="text-decoration: none;" href="{{ $contentUrl }}"><strong style="color: #990000">{{ $contentTitle }}</strong></a> content has been changed to <i><strong style='color: #6da5ff;'>{{{ $taskName }}}</strong></i>. The due date has also changed from {{ $orignalTaskDueDate }} to <strong>{{ $taskDueDate }}</strong>.
										@endif
									</td>
								</tr>
